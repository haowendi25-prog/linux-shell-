...
about_project() {
    clear
    show_brand
    echo -e "  ${BOLD}${YELLOW}▸ 关于 DiagMaster${NC}"
    echo -e "  ${DGRAY}──────────────────────────────────────────────${NC}"
    echo ""
    echo -e "  ${CYAN}项目愿景${NC}"
    echo -e "  为 Linux 运维人员提供一键式多维诊断能力，降低服务器排障门槛，"
    echo -e "  将\"人工巡检\"转化为\"自动化、可视化、可审计\"的标准化流程。"
    echo ""
    echo -e "  ${CYAN}核心特性${NC}"
    echo -e "  ${GREEN}✓${NC} 零依赖纯 Bash 实现，Ubuntu / WSL 开箱即用"
    echo -e "  ${GREEN}✓${NC} 分布式 SSH 节点管理，支持多服务器远程诊断"
    echo -e "  ${GREEN}✓${NC} 多进程并行采集，性能数据实时汇总"
    echo -e "  ${GREEN}✓${NC} 内核日志清洗 + SSH 爆破检测，JSON / Markdown 双格式报告"
    echo -e "  ${GREEN}✓${NC} 智能磁盘清理 + apt fix-broken + 服务重启自愈"
    echo -e "  ${GREEN}✓${NC} systemd 守护进程封装，支持定时自动巡检"
    echo ""
    echo -e "  ${CYAN}技术栈${NC}"
    echo -e "  Bash 5.x | grep/awk/sed | ncurses (whiptail) | systemd | nc | ssh"
    echo ""
    echo -e "  ${CYAN}工程化实践${NC}"
    echo -e "  • 配置与代码分离 (diag.conf)"
    echo -e "  • 模块化单体架构 (主入口 + 3 个业务模块 + 公共库)"
    echo -e "  • 31 个自动化测试用例 (功能 / 异常 / 边界 / 集成)"
    echo -e "  • Makefile 安装入口，make install / make clean"
    echo -e "  • JSON / Markdown 双格式审计报告输出"
    echo ""
    echo -e "  ${CYAN}架构概览${NC}"
    echo -e "  ${DGRAY}┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐${NC}"
    echo -e "  ${DGRAY}│   diagmaster.sh │◄──►│   modules/*.sh  │◄──►│   lib/utils.sh  │${NC}"
    echo -e "  ${DGRAY}│   (主入口/UI)   │    │  (业务逻辑)     │    │  (SSH/工具函数) │${NC}"
    echo -e "  ${DGRAY}└─────────────────┘    └─────────────────┘    └─────────────────┘${NC}"
    echo -e "  ${DGRAY}         │                       │                       │${NC}"
    echo -e "  ${DGRAY}         ▼                       ▼                       ▼${NC}"
    echo -e "  ${DGRAY}  ┌─────────────┐      ┌─────────────┐      ┌─────────────┐${NC}"
    echo -e "  ${DGRAY}  │  data/      │      │  logs/      │      │  reports/   │${NC}"
    echo -e "  ${DGRAY}  │  (节点/缓存) │      │  (活动/守护) │      │  (审计/巡检) │${NC}"
    echo -e "  ${DGRAY}  └─────────────┘      └─────────────┘      └─────────────┘${NC}"
    echo ""
    echo -e "  ${CYAN}团队${NC}: 翟浩雯 (组长) | 李薇 (组员)"
